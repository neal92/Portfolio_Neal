...
const Projects: React.FC = () => {
  ...
  <div className="relative z-10 grid gap-12 grid-cols-1 md:grid-cols-2">
          {projects.map((project, index) => (
            <div
              key={index}
              className={`group ${project.id === 'service-booking' ? 'bg-slate-900 text-white border-slate-800' : 'bg-white/80'} backdrop-blur-xl rounded-3xl overflow-hidden border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 flex flex-col md:flex-row`}
            >
              <div className="w-full p-6 sm:p-10 flex flex-col justify-center h-full">
                <div>
                  <h3 className={`text-3xl font-bold mb-4 group-hover:text-blue-400 transition-colors duration-300 ${project.id === 'service-booking' ? 'text-white' : 'text-slate-800'}`}>{project.title}</h3>
                  <p className={`mb-6 leading-relaxed text-base sm:text-lg ${project.id === 'service-booking' ? 'text-slate-300' : 'text-slate-600'}`}>{project.description}</p>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-2xl w-full p-8 relative flex flex-col items-center">
          ...
          {projects[openModal].video ? (
            ...
            <div className="flex flex-col items-center justify-center w-full h-64">
              <span className="text-2xl font-semibold text-slate-700 dark:text-slate-200 mb-4">Aucune vidéo disponible pour ce projet.</span>
              <span className="text-base text-slate-500 dark:text-slate-400">Contactez-moi pour une démo personnalisée !</span>
            </div>
          )}
        </div>
      </div>
