...
const Skills: React.FC<SkillsProps> = ({ language }) => {
  ...
  return (
    <div className="py-32 bg-gradient-to-b from-white to-slate-50 relative overflow-hidden">
      {/* Background decorations */}
      <div className="absolute inset-0">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-br from-blue-100 to-purple-100 rounded-full blur-3xl opacity-20"></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-gradient-to-br from-pink-100 to-orange-100 rounded-full blur-3xl opacity-20"></div>
      </div>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <div className="inline-block p-1 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full mb-8">
        ...
        <div className="relative z-10 grid lg:grid-cols-2 gap-12 mb-20">
          <div className="group bg-white/60 backdrop-blur-xl p-10 rounded-3xl border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
        ...
        <div className="relative z-10 bg-white/60 backdrop-blur-xl p-10 rounded-3xl border border-white/20 shadow-xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {tools.map((tool, index) => (
              <div key={index} className="flex flex-col items-center justify-center p-4 bg-white rounded-xl shadow hover:shadow-lg transition">
                ...
                <span className="text-lg font-medium text-slate-700">{tool.name}</span>
              </div>
            ))}
          </div>
        </div>
